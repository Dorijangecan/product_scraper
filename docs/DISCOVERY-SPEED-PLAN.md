...
| 4 | **D2** tvrdi rok + degradacija | ⏳ **DJELOMIČNO (D2a + D2b)** — odabrana opcija **(B)**; `ScrapeContext.deadline` (meki cilj 30 s / tvrdi strop 4 min) proslijeđen po artiklu, pipeline prestaje nagađati, browser gated na stropu, discovery timeouti stisnuti na ostatak, svaki rez ima razlog. **OTVORENO:** dokumenti (jedan PDF može pojesti ~220 s) | srednji |
