...
const catalogNumber = process.argv[2] ?? "1SDA126190R1";
const manufacturer = getManufacturerConfig("abb");
if (!manufacturer) throw new Error("ABB manufacturer not configured");
...
try {
  console.log(`\n=== ABB scrape probe: ${catalogNumber} ===\n`);
  const scraped = await connector.scrape(catalogNumber, {
    http,
    manufacturer,
    runDir: rootDir,
    documentsDir: path.join(rootDir, "benchmarks", "output", "probe-abb"),
    signal: undefined,
    browserRenderer,
    learnedEndpoints: {
  ...
  console.log(`\n--- normalized fields (${Object.keys(result.normalized).length}) ---`);
  console.dir(result.normalized, { depth: 4 });
  ...
  for (const doc of result.documents) {
    console.log(`  ${doc.type.padEnd(11)} ${doc.label} -> ${doc.url}`);
  }

  console.log(`\n--- quality gate ---`);
  console.dir(result.qualityGate, { depth: 4 });
