...
const catalogNumber = process.argv[2] ?? "P1-25/I2/SVB";
const manufacturer = getManufacturerConfig("eaton");
if (!manufacturer) throw new Error("Eaton manufacturer not configured");
...
try {
  console.log(`\n=== Eaton scrape probe: ${catalogNumber} ===\n`);
  const scraped = await connector.scrape(catalogNumber, {
    http,
    manufacturer,
    runDir: rootDir,
    documentsDir: path.join(rootDir, "benchmarks", "output", "probe-eaton"),
    signal: undefined,
    browserRenderer,
    learnedEndpoints: {
