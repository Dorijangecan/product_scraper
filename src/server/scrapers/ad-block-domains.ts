/**
 * Static ad/tracker domain blocklist for the Playwright browser-fallback renderer.
 *
 * Source: Peter Lowe's ad and tracking server list (https://pgl.yoyo.org/adservers/,
 * hostformat=plain), the same public list Scrapling's StealthyFetcher vendors for its
 * `block_ads` option (scrapling/engines/toolbelt/ad_domains.py). Snapshot taken 2026-09.
 * Regenerate by re-fetching that file and re-running the extraction (lowercase + dedupe);
 * do not hand-edit entries below.
 */
export const AD_BLOCK_DOMAINS: ReadonlySet<string> = new Set([
  "101com.com",
  "180hits.de",
...
/** True if `hostname` (or one of its parent domains) is a known ad/tracker host. */
export function isAdBlockedHost(hostname: string): boolean {
  const labels = hostname.toLowerCase().split(".");
  // Walk from the full hostname down to (but excluding) the bare TLD, so a match on
  // "tracker.ads.doubleclick.net" also fires via its "doubleclick.net" parent, without
  // ever treating a single-label suffix ("net") as a match.
  for (let i = 0; i < labels.length - 1; i += 1) {
    if (AD_BLOCK_DOMAINS.has(labels.slice(i).join("."))) return true;
  }
  return false;
}

/** True if the URL's host should be blocked by the ad/tracker filter. Malformed URLs pass through unblocked. */
export function isAdBlockedUrl(url: string): boolean {
  try {
    return isAdBlockedHost(new URL(url).hostname);
  } catch {
    return false;
  }
}
