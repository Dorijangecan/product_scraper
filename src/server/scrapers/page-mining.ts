...
import { isPlausibleSpecPair } from "./spec-plausibility.js";
import { looksLikeUnderstandableSpec, matchPropertyPrefix } from "./ontology.js";
import { matchRegistryFieldLabelPrefix } from "./field-registry.js";
import { readHtmlTableAttributes, replayHtmlTableHeaderColumn } from "./html-table-reader.js";
...
/** Replay only an intentionally tiny selector grammar; learned data is a recipe, never arbitrary
 * selector input. Its values still pass `pushAttribute`'s normal usefulness gate. */
function replayLearnedTableRows(
  $: cheerio.CheerioAPI,
  learnedPatterns: string[] | undefined,
  pushAttribute: (group: string, name: string, value: string, parser?: string) => boolean,
  signals: Set<string>
) {
  for (const pattern of [...new Set(learnedPatterns ?? [])].slice(0, 12)) {
    const selector = tableRowSelectorFromRecipe(pattern);
    if (!selector) continue;
    try {
      $(selector).slice(0, 160).each((_, element) => {
        const cells = $(element).find("th,td").map((__, cell) => cleanText($(cell).text())).get().filter(Boolean);
        if (cells.length >= 2 && pushAttribute("Learned Table", cells[0], cells.slice(1).join(" | "))) {
          signals.add(`replayed:${pattern}`);
        }
      });
    } catch {
      // A site can remove a class/id between runs; fall through to the generic sweep.
    }
  }
}

/**
 * A learned HTML-table recipe names one stable table and one actual column header. It is not a
 * free CSS selector: the selector grammar is the same tiny id/class subset generated by the
 * table reader, and the encoded header must round-trip through `cleanText`. The table reader then
 * proves the requested catalog's variant again before returning any value.
 */
function replayLearnedHtmlTableHeaderColumns(
  $: cheerio.CheerioAPI,
  learnedPatterns: string[] | undefined,
  catalogNumber: string,
  sourceUrl: string,
  ...
  for (const pattern of [...new Set(learnedPatterns ?? [])].slice(0, 12)) {
    const recipe = htmlTableHeaderColumnRecipe(pattern);
    if (!recipe) continue;
    let replayed = false;
    for (const attribute of replayHtmlTableHeaderColumn($, catalogNumber, sourceUrl, recipe.selector, recipe.header)) {
      if (pushAttribute("Learned HTML Table", attribute.name, attribute.value, "page-mining-html-table-replay")) replayed = true;
    }
    if (replayed) signals.add(`replayed:${pattern}`);
  }
}

function tableRowSelectorFromRecipe(pattern: string): string | undefined {
  const prefix = "css:table-row:";
  if (!pattern.startsWith(prefix)) return undefined;
  const selector = pattern.slice(prefix.length);
  return /^tr(?:#[A-Za-z][\w-]{0,79}|(?:\.[A-Za-z_-][\w-]{0,79}){1,2})$/.test(selector) ? selector : undefined;
}

/** A JSON recipe may name one stable script ID, never a general CSS selector or a JSONPath expression.
 * The normal embedded-JSON reader still decides which values are useful; the learned part only says
 * where its already-proven input lives. */
function replayLearnedJsonScripts(
  $: cheerio.CheerioAPI,
  learnedPatterns: string[] | undefined,
  catalogNumber: string,
  pushAttribute: (group: string, name: string, value: string, parser?: string) => boolean,
  ...
  for (const pattern of [...new Set(learnedPatterns ?? [])].slice(0, 12)) {
    const selector = jsonScriptSelectorFromRecipe(pattern);
    if (!selector) continue;
    const text = $(selector).first().html() ?? "";
    if (!text || text.length > 900_000) continue;
    if (mineJsonChunk(text, catalogNumber, pushAttribute, pushDocument, signals, "Learned JSON")) {
      signals.add(`replayed:${pattern}`);
    }
  }
}

function jsonScriptRecipe(id: string): string | undefined {
  return /^[A-Za-z_][\w-]{0,79}$/.test(id) ? `json:script:#${id}` : undefined;
}

function jsonScriptSelectorFromRecipe(pattern: string): string | undefined {
  const prefix = "json:script:#";
  if (!pattern.startsWith(prefix)) return undefined;
  const id = pattern.slice(prefix.length);
  return jsonScriptRecipe(id) === pattern ? `script#${id}` : undefined;
}

function htmlTableHeaderColumnRecipe(pattern: string): { selector: string; header: string } | undefined {
  ...
  if (!/^table(?:#[A-Za-z][\w-]{0,79}|(?:\.[A-Za-z_-][\w-]{0,79}){1,2})$/.test(selector)) return undefined;
  try {
    const header = decodeURIComponent(rest.slice(separator + 1));
    return header && header.length <= 180 && cleanText(header) === header ? { selector, header } : undefined;
  } catch {
    return undefined;
  }
}

/** Only persist patterns that a later run can execute, plus bounded element-cap hints. */
export function isPersistableLearnedPattern(pattern: string): boolean {
  return Boolean(tableRowSelectorFromRecipe(pattern) || jsonScriptSelectorFromRecipe(pattern) || htmlTableHeaderColumnRecipe(pattern))
    || /^capped:(?:hidden-dom|data-attributes|key-value-table|embedded-json-scripts)$/.test(pattern);
}

function stableTableRowSelector(element: unknown): string | undefined {
  const attrs = (element as { attribs?: Record<string, string> }).attribs ?? {};
  const id = cleanText(attrs.id ?? "");
  if (/^[A-Za-z][\w-]{0,79}$/.test(id)) return `tr#${id}`;
  const classes = cleanText(attrs.class ?? "")
    .split(/\s+/)
    .filter((value) => /^[A-Za-z_-][\w-]{0,79}$/.test(value))
    .slice(0, 2);
  return classes.length ? `tr.${classes.join(".")}` : undefined;
}
...
export function minePage(fetched: FetchedText, options: PageMiningOptions): PageMiningResult {
  ...
  const pushAttribute = (group: string, name: string, value: string, parser = "page-mining") => {
    ...
    if (!isUsefulAttribute(cleanName, cleanValue)) return false;
    attributes.push({
      group,
  ...
  replayLearnedTableRows($, options.learnedPatterns, pushAttribute, signals);
  replayLearnedHtmlTableHeaderColumns($, options.learnedPatterns, options.catalogNumber, fetched.effectiveUrl, pushAttribute, signals);
  replayLearnedJsonScripts($, options.learnedPatterns, options.catalogNumber, pushAttribute, pushDocument, signals);
  ...
  mineSemanticKeyValueShapes($, options.catalogNumber, fetched.effectiveUrl, pushAttribute, signals, caps.keyValueTable);
...
function mineSemanticKeyValueShapes(
  ...
  const matrixTables = readHtmlTableAttributes($, catalogNumber, sourceUrl);
  const acceptedMatrixAttributes = new Set<string>();
  for (const attribute of matrixTables.attributes) {
    if (pushAttribute("Mined HTML Table", attribute.name, attribute.value, "page-mining-html-table")) {
      acceptedMatrixAttributes.add(minedAttributeKey(attribute.name, attribute.value));
    }
  }
  for (const recipe of matrixTables.recipes) {
    if (recipe.attributes.some((attribute) => acceptedMatrixAttributes.has(minedAttributeKey(attribute.name, attribute.value)))) {
      signals.add(`html-table:header-column:${recipe.selector}:${encodeURIComponent(recipe.header)}`);
    }
  }
  ...
  tableRows.slice(0, cap).each((_, element) => {
    if (matrixTables.handledTables.has($(element).closest("table").get(0))) return;
    const cells = $(element).find("th,td").map((__, cell) => cleanText($(cell).text())).get().filter(Boolean);
    if (cells.length >= 2) {
      signals.add("key-value-table");
      if (pushAttribute("Mined Table", cells[0], cells.slice(1).join(" | "))) {
        const selector = stableTableRowSelector(element);
        if (selector) signals.add(`css:table-row:${selector}`);
      }
    }
  });
...
function mineEmbeddedJson(
  ...
  const chunks: Array<{ text: string; recipe?: string }> = [];
  ...
  scripts.slice(0, cap).each((_, element) => {
    ...
    if (/json|ld\+json/i.test(type) || /NEXT_DATA|NUXT|apollo|redux|state|product|data/i.test(id) || PRODUCT_JSON_KEYS.test(text.slice(0, 5000))) {
      chunks.push({ text, recipe: jsonScriptRecipe(id) });
    }
  });
  chunks.push(...extractAssignedJsonChunks(html).slice(0, 30).map((text) => ({ text })));
  chunks.push(...extractJsonAttributeChunks($).slice(0, 60).map((text) => ({ text })));

  for (const chunk of chunks.slice(0, 80)) {
    const emittedAttribute = mineJsonChunk(chunk.text, catalogNumber, pushAttribute, pushDocument, signals, "Embedded JSON");
    if (emittedAttribute && chunk.recipe) signals.add(chunk.recipe);
  }
}

function minedAttributeKey(name: string, value: string): string {
  return `${cleanText(name).toLocaleLowerCase()}\u0000${cleanText(value).toLocaleLowerCase()}`;
}

/** Parse one already-selected JSON source. Returning true means this source, not merely another
 * sweep on the page, emitted an admissible attribute and is safe to remember as a recipe. */
function mineJsonChunk(
  chunk: string,
  catalogNumber: string,
  pushAttribute: (group: string, name: string, value: string, parser?: string) => boolean,
  pushDocument: (url: string | undefined, label: string, context?: string) => void,
  signals: Set<string>,
  group: "Embedded JSON" | "Learned JSON"
): boolean {
  let emittedAttribute = false;
  const trackedPushAttribute = (attributeGroup: string, name: string, value: string, parser?: string) => {
    const accepted = pushAttribute(attributeGroup, name, value, parser);
    emittedAttribute ||= accepted;
    return accepted;
  };
  for (const value of parseJsonCandidates(chunk).slice(0, 10)) {
    ...
    flattenJsonValue(value, [], {
      catalogNumber,
      group,
      pushAttribute: trackedPushAttribute,
      pushDocument,
      seen: new Set()
    });
  }
  return emittedAttribute;
}
...
function flattenJsonValue(
  ...
  if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") {
    ...
    if (PRODUCT_JSON_KEYS.test(label) || USEFUL_LABEL.test(label) || USEFUL_VALUE.test(stringValue) || catalogTextMatches(stringValue, context.catalogNumber, { compact: true, ignoreCase: true, afterColon: true })) {
      context.pushAttribute(context.group, label, stringValue, "page-mining-json");
    }
    return;
  }
  ...
  if (typeof value === "object") {
    ...
    if (pairName && pairValue && isUsefulAttribute(pairName, pairValue)) {
      context.pushAttribute(context.group, pairName, pairValue, "page-mining-json");
    }
...
function textPairs(text: string): Array<{ name: string; value: string }> {
  ...
  pairs.push(...ontologyInlinePairs(inline));
...
/** Split `Label: value Label: value` prose by ontology labels, not an English allowlist. */
function ontologyInlinePairs(text: string): Array<{ name: string; value: string }> {
  const delimiters = [...text.matchAll(/[:=]/g)].map((match) => match.index ?? -1).filter((index) => index >= 0);
  const labels = delimiters.flatMap((delimiterIndex) => {
    const label = ontologyLabelBefore(text, delimiterIndex);
    return label ? [{ ...label, delimiterIndex }] : [];
  });
  return labels.flatMap((label, index) => {
    const end = labels[index + 1]?.start ?? Math.min(text.length, label.delimiterIndex + 1 + 180);
    const value = cleanInlineValue(text.slice(label.delimiterIndex + 1, end));
    return value ? [{ name: label.name, value }] : [];
  });
}

function ontologyLabelBefore(text: string, delimiterIndex: number): { name: string; start: number } | undefined {
  const windowStart = Math.max(0, delimiterIndex - 180);
  const before = text.slice(windowStart, delimiterIndex).replace(/\s+$/, "");
  let best: { name: string; start: number; end: number } | undefined;
  for (const match of before.matchAll(/(?:^|[\s;|•])(?=\p{L})/gu)) {
    const start = (match.index ?? 0) + match[0].length;
    const found = matchPropertyPrefix(before.slice(start)) ?? matchRegistryFieldLabelPrefix(before.slice(start));
    if (!found) continue;
    const end = start + found.length;
    if (!best || end > best.end || (end === best.end && found.length > best.name.length)) {
      best = { name: cleanText(before.slice(start, end)), start: windowStart + start, end };
    }
  }
  return best && best.name ? { name: best.name, start: best.start } : undefined;
}
...
function isUsefulAttribute(name: string, value: string): boolean {
  ...
  // Third choke point for the shared "is this a specification at all" gate (the others are
  // stampDocumentAttributes and parseGenericProductPage). Mining reads raw DOM/JSON, so it is the
  // one path that can turn a layout hook into a measurement: `data-height-onload="30"` used to
  // reach the export as an enclosure's "H 30" dimensions.
  if (!isPlausibleSpecPair(cleanName, cleanValue)) return false;
  return looksLikeUnderstandableSpec(cleanName, cleanValue) || USEFUL_LABEL.test(cleanName) || USEFUL_VALUE.test(cleanValue) || PRODUCT_JSON_KEYS.test(cleanName);
}
