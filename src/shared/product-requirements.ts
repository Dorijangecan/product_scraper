...
const NON_ELECTRICAL_ACCESSORY_PATTERN =
  /\b(cable\s+(?:gland|entry|fitting|duct|tray|tie|clamp|mount)|mount(?:ing)?\s+(?:kit|foot|bracket|plate|hardware)|sub\s*panel|subpanel|back\s*panel|dead\s*front|terminal\s+covers?|trip\s+unit\s+cover|key\s+locks?|padlocks?|interlocks?|lifting\s+plate|mounting\s+flange|support\s+(?:plate|bracket)|cover\s+(?:kit|plate|accessor(?:y|ies))|hinge|latch|gasket|window\s+kit|adapter\s+plate|shelf|rail|duct|wireway|enclosure\s+accessor(?:y|ies))\b/i;
